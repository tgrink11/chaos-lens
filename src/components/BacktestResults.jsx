...
function rateColor(rate) {
  if (rate == null) return 'text-[#667085]';
...
function rateBg(rate) {
  ...
  if (rate >= 0.55) return '#0a7a2e';
  if (rate >= 0.40) return '#b45309';
  return '#912323';
}
...
function HitRateCard({ label, metrics }) {
  ...
  return (
    <div className="flex-1 bg-[#f0f2f5] rounded-xl p-5">
      <div className="text-xs text-[#667085] uppercase tracking-wider font-semibold mb-2">
        {label}
      </div>
            ...
            data?.total > 0 && directions.map((dir, i) => {
              ...
              return (
                <tr key={`${label}-${dir}`} className="border-t border-chaos-600">
                  <td className="py-2 pr-4 text-[#344054] font-mono">
                    {i === 0 ? label : ''}
                  </td>
                  <td className={`py-2 pr-4 capitalize ${DIR_COLORS[dir]}`}>
                    {dir}
                  </td>
                  <td className="py-2 pr-4 text-right text-[#344054] font-mono">
                    {d.predicted}
                  </td>
                  <td className="py-2 pr-4 text-right text-[#344054] font-mono">
                    {d.correct}
...
function CalibrationChart({ calibration, label }) {
  ...
  return (
    <div>
      <div className="text-xs text-[#667085] uppercase tracking-wider font-semibold mb-3">
        {label} — Confidence Calibration
      </div>
      <div className="space-y-2">
        {calibration.map(bucket => (
          <div key={bucket.label} className="flex items-center gap-3">
            <div className="w-16 text-xs text-[#667085] font-mono text-right shrink-0">
              {bucket.label}
            </div>
            ...
            <div className="w-24 text-xs text-[#667085] font-mono shrink-0">
              {bucket.count > 0
                ? <><span className={rateColor(bucket.hitRate)}>{pct(bucket.hitRate)}</span> <span className="text-[#98a2b3]">({bucket.count})</span></>
...
function TestPointRow({ point }) {
  ...
  return (
    <tr className="border-t border-chaos-700 text-xs">
      <td className="py-1.5 pr-3 text-[#667085] font-mono">{point.date}</td>
      <td className={`py-1.5 pr-3 capitalize font-mono ${DIR_COLORS[point.predicted15.direction]}`}>
        {point.predicted15.direction} <span className="text-[#667085]">{point.predicted15.confidence}%</span>
      </td>
      ...
      <td className={`py-1.5 pr-3 capitalize font-mono ${DIR_COLORS[point.predicted62.direction]}`}>
        {point.predicted62.direction} <span className="text-[#667085]">{point.predicted62.confidence}%</span>
      </td>
...
export default function BacktestResults({ backtestResult, symbol, loading, progress }) {
  ...
  if (loading) {
    return (
      <div className="bg-chaos-800 rounded-xl p-6 border border-chaos-600 no-print">
        <h2 className="text-lg font-semibold text-[#1a1a1a] mb-3 font-mono">Backtest</h2>
        ...
        <div className="h-1.5 bg-chaos-700 rounded-full overflow-hidden mt-3">
          <div
            className="h-full bg-fractal-cyan rounded-full transition-all duration-300"
  ...
  if (backtestResult?.error) {
    return (
      <div className="bg-chaos-800 rounded-xl p-6 border border-chaos-600 no-print">
        <h2 className="text-lg font-semibold text-[#1a1a1a] mb-2 font-mono">Backtest</h2>
        <p className="text-sm text-[#667085]">{backtestResult.message}</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-chaos-800 rounded-xl p-6 border border-chaos-600 no-print">
      ...
      <div className="flex items-center justify-between mb-1">
        <h2 className="text-lg font-semibold text-[#1a1a1a] font-mono">
          Backtest Results
        </h2>
        <span className="text-xs text-[#667085] font-mono bg-chaos-700 px-2 py-1 rounded">
      ...
      <div>
        <button
          ...
          className="text-xs text-[#667085] hover:text-[#1a1a1a] transition-colors font-mono"
        >
        ...
        {showTestPoints && (
          <div className="mt-3 max-h-80 overflow-y-auto border border-chaos-600 rounded-lg">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-xs text-[#667085] uppercase tracking-wider sticky top-0 bg-chaos-800">
